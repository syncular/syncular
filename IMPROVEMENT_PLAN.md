...
Related: the `*-hono.wasm.test.ts` suites fail locally with worker fetch
"Unable to connect" (48/62 on the dedicated gate) while CI presumably passes —
suspect local Bun 1.3.14 vs pinned `bun@1.3.9`. Reproduce under the pinned Bun
and either fix or enforce the version (volta/mise pin) so local == CI.
...
→ WASM runtime. Knip also reports zero unused files. The implementation plan
(`rust/docs/implementation history`) treats this TS worker bridge as the accepted
...
Deferred (per implementation plan: WP-14 DX work reopens only on concrete app feedback):
...
### 10. Decide the umbrella-package story

`syncular` (31 passthrough re-export files) vs `@syncular/*`: docs use scoped
names, npm search surfaces the umbrella. Pick one (recommend: deprecate
umbrella re-exports, keep it as the CLI/codegen entry only) and align docs.
...
### 12. Default-path snapshot encoder metadata caching

Generic snapshot encode does per-row type inference
(`packages/server/src/snapshot-artifacts.ts`); measured ~500–650ms each for
query+encode at 500k rows (BENCHMARK_LOG). Precompute per-table column
metadata at schema registration so the non-codegen path stops re-inferring.
