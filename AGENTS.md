...
- Tests: `bun test`
- Demo (smoke test manually in browser): `bun --cwd demo dev`
- Docs site: `bun --cwd apps/docs dev` (build: `bun --cwd apps/docs build`)
- Console UI: `bun --cwd console dev` (build: `bun --cwd console build`)
...
- Server table handler interface: `packages/server/src/shapes/types.ts`
- Subscription resolver interface: `packages/server/src/subscriptions/types.ts`
...
- React provider lifecycle: `packages/client-react/src/context.tsx`
...
- Run `bun check:fix`
- Run `bun test`
- Run the demo (`bun --cwd demo dev`) and do a quick manual smoke test in a browser
