...
## RFC 0005 own-JS budget re-derivation (2026-09-19)

RFC 0005 previous-version context raised the anti-bloat tripwire
(`ownJsRawCeilingBytes`) past its 130 KB line, so the ceiling is re-derived
under the standing rule — measure, attribute, re-pin at ~5% headroom (the
118 → 122 → 130 KB precedent). The measurement is unchanged and still
conservative: one unsplit `Bun.build({minify: true})` over
`bench/src/bundle-entry.ts` (target browser, `conditions: ['bun']`,
`@sqlite.org/sqlite-wasm` external), which counts dynamically imported
opt-in modules on purpose.

| Tree | Raw | Gzip |
| --- | ---: | ---: |
| base `8b22d819` | 131,294 | 38,541 |
| RFC 0005 head | 143,975 | 42,128 |
| delta | +12,681 | +3,587 |

Minified attribution of the delta
(`metafile.outputs[entry].inputs[].bytesInOutput`):
...
wiring and `previousVersionSnapshot` / `previousVersionAudit` /
...
New ceiling: 143,975 × 1.05 = 151,173.75 bytes = 147.63 KiB, rounded up to
**148 KB** (5.26% headroom). `totalGzipCeilingBytes` (600 KB) and every
performance budget are unchanged.

## Sparse patch payloads: 500 patches over twenty columns (2026-09-16)

RFC-WRITE-SEMANTICS §11. `bench/src/patch-lane.ts` queues 500 `patch` calls,
each writing two of twenty columns on a distinct seeded row, then drains them
in one sync round on the bun:sqlite loopback lane. The pre-flip tree
(`def459ef`) sends the full twenty-column row for every patch; the shipped
sparse client (`fe4fc1e2`) sends a presence bitmap naming the primary key and
the two written columns. The drain coalesces all 500 commits into one push
request. Request bytes are deterministic; drain time varies.

| Metric | Before (`def459ef`) | After (`fe4fc1e2`) | Delta |
| --- | ---: | ---: | ---: |
| Request bytes, 500 patches | 123,756 | 57,866 | **-53.2%** |
| Response bytes | 170,646 | 170,646 | 0 |
| Drain time, median | 35.8 ms | 38.1 ms | +6.4% |
| Drain time, range | 34.1 to 49.5 ms | 36.2 to 49.9 ms | overlapping |

Six fresh trials per side (two invocations of three trials, baseline and
candidate alternating on one host; Apple M4, Bun 1.4.0). Request and response
bytes repeat exactly across trials. The drain-time medians differ by 2.3 ms
while each side's own spread is about 14 ms, so the timing difference sits
below the lane's run-to-run noise; the byte reduction is exact. Response bytes
are identical because `COMMIT` delivery keeps the full-row codec. The 0.1 ms
write-ack p50 in section b measures a single-row push, so this lane's absolute
times are not comparable to it.
