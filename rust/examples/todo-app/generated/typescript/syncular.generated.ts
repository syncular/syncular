...
import { SYNCULAR_V2_PACKAGE_NAME, SYNCULAR_V2_PACKAGE_VERSION, SYNCULAR_V2_WORKER_PROTOCOL_VERSION, createSyncularRustSqliteDatabase } from '../../../../bindings/browser/src';
import type { CreateSyncularRustSqliteDatabaseOptions, SyncularRustSqliteDatabase, SyncularV2FieldEncryptionConfig, SyncularV2FieldEncryptionRule, SyncularV2RuntimeInfo, SyncularYjsPayloadEnvelope } from '../../../../bindings/browser/src';
...
export function assertSyncularAppRuntimeInfo(runtimeInfo: SyncularV2RuntimeInfo): void {
  ...
  if (runtimeInfo.rust.schemaVersion !== syncularGeneratedSchemaVersion) {
    throw new Error(`Syncular Rust schema version mismatch: ${runtimeInfo.rust.schemaVersion}, expected ${syncularGeneratedSchemaVersion}`);
  }
  if (!runtimeInfo.rust.features.includes('web-owned-sqlite')) {
...
export async function createSyncularAppDatabase(
  ...
  try {
    ...
    await ensureSyncularAppSchema(database.db);
