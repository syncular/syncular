...
pub trait BorrowedBinarySnapshotCellVisitor<'a> {
...
impl<'a> BinarySnapshotRowCursor<'a> {
        ...
        F: FnMut(usize, &BinarySnapshotColumn, BorrowedBinarySnapshotCell<'a>) -> Result<()>,
    {
        ...
        for (column_index, column) in self.columns.iter().enumerate() {
            ...
            on_cell(column_index, column, value)?;
        }
        self.remaining -= 1;
        Ok(true)
    }

    pub fn read_next_row_values<F>(&mut self, mut on_cell: F) -> Result<bool>
    where
        F: FnMut(BorrowedBinarySnapshotCell<'a>) -> Result<()>,
    {
        if self.remaining == 0 {
            return Ok(false);
        }
        ...
        for (column_index, column) in self.columns.iter().enumerate() {
            ...
            if is_null {
                ...
                on_cell(BorrowedBinarySnapshotCell::Null)?;
                continue;
            }
            let value = self
                .reader
    ...
    pub fn read_next_row_with_visitor<V>(&mut self, visitor: &mut V) -> Result<bool>
    where
        V: BorrowedBinarySnapshotCellVisitor<'a>,
    {
        if self.remaining == 0 {
            return Ok(false);
        }
        ...
        V: BorrowedBinarySnapshotCellVisitor<'a>,
    {
        match column_type {
            BinarySnapshotColumnType::String => {
                visitor.visit_string(self.read_str32("binary snapshot string")?)
            }
...
mod tests {
    ...
    fn decodes_binary_snapshot_table_rows() {
        ...
        struct RecordingVisitor<'a> {
            values: Vec<BorrowedBinarySnapshotCell<'a>>,
        }

        impl<'a> BorrowedBinarySnapshotCellVisitor<'a> for RecordingVisitor<'a> {
            fn visit_null(&mut self) -> Result<()> {
                self.values.push(BorrowedBinarySnapshotCell::Null);
                Ok(())
            }

            fn visit_string(&mut self, value: &'a str) -> Result<()> {
                self.values.push(BorrowedBinarySnapshotCell::String(value));
                Ok(())
            }

            fn visit_integer(&mut self, value: i64) -> Result<()> {
                self.values.push(BorrowedBinarySnapshotCell::Integer(value));
                Ok(())
            }

            fn visit_float(&mut self, value: f64) -> Result<()> {
                self.values.push(BorrowedBinarySnapshotCell::Float(value));
                Ok(())
            }

            fn visit_boolean(&mut self, value: bool) -> Result<()> {
                self.values.push(BorrowedBinarySnapshotCell::Boolean(value));
                Ok(())
            }

            fn visit_json(&mut self, value: &'a str) -> Result<()> {
                self.values.push(BorrowedBinarySnapshotCell::Json(value));
                Ok(())
            }

            fn visit_bytes(&mut self, value: &'a [u8]) -> Result<()> {
                self.values.push(BorrowedBinarySnapshotCell::Bytes(value));
                Ok(())
            }
        }
