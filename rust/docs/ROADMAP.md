...
- `[~]` [`WP-50 Syncular DX Rough Edges`](work-packages/WP-50-syncular-dx-rough-edges.md)
  - In progress. Retained slices add an app-facing
    `getSyncularBrowserHealth(...)` helper over existing diagnostics/status
    data, wire the `create-syncular-app` starter to show storage durability,
    subscription readiness, and realtime state, assert the helper in the fresh
    ...
    metrics without recording hashes, signed URLs, direct-transfer tokens, or
    payload text.
    ...
    passed. The current starter smoke slice adds a smoke-only same-origin
    ...
    hosted Chrome.
    The PWA follow-up also adds smoke-only service-worker update activation
    ...
    scaffold smoke, and diff check passed; hosted Checks run `28566859795` on
    ...
    effect, and only then mounts the task pane. The starter task list uses a
    plain query plus `rowsChanged` refetch instead of the live-query path so
    browser preview startup does not depend on live-query registration, while a
    hidden `starterOpen` phase/diagnostic marker records the phase trail for
    Chrome/CDP artifacts.
    ...
    same-context fresh-page, and persistent-profile relaunch execution without
    ...
    context in both diagnostic details and transport stats, lifecycle Web Lock
    name/state/timeout evidence, pause reason/visibility state, and dropping
    the page text excerpt before storing a `browser.preview_failure` record.
    ...
    smoke self-checks that artifact shape plus preview/asset metrics without
    requiring Chrome, and the hosted starter browser-preview job uploads that
    JSON on failure from a predictable `.context/starter-browser-preview-smoke`
    directory.
