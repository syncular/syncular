...
| `json-v1` sync-pack path | `Temporary` | Explicit sync-pack negotiation in `packages/core/src/sync-packs.ts`, `rust/crates/protocol/src/binary_sync_pack.rs`, protocol fixtures | Existing JSON sync-pack encoding and fixture coverage | Explicit JSON requests can keep old combined-response encoding alive beside binary sync-packs | Unspecified combined sync responses now default to `binary-sync-pack-v1`. Decide whether server/core should keep JSON sync-pack as a test/debug format or remove it entirely after protocol fixtures move to binary-only current paths |
| `json-row-frame-v1` snapshot chunks | `Temporary` | Explicit server/core snapshot chunk negotiation, `packages/core/src/snapshot-chunks.ts`, `rust/crates/protocol`, protocol fixtures | Existing snapshot chunk format and fixture coverage | Explicit JSON row-frame requests can keep the old format alive beside binary-table chunks | Rust runtime transports no longer accept JSON row-frame chunks, and unspecified server pulls default to `binary-table-v1`. Decide whether server/core should keep JSON row-frame as a test/debug format or remove it entirely when binary/artifact paths cover all non-Rust clients |
...
| Server default JSON sync-pack responses | `Removed` | `packages/core/src/sync-packs.ts`, Hono sync route tests | Unspecified combined sync requests now prefer `binary-sync-pack-v1`; JSON sync-pack output is only used when explicitly requested by tests/debug callers |
