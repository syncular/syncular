...
| `json-row-frame-v1` snapshot chunks | `Temporary` | Server snapshot chunk default/docs/tests, `packages/core/src/snapshot-chunks.ts`, `rust/crates/protocol`, protocol fixtures | Existing server/core snapshot chunk format and fixture coverage | Server defaults can keep JSON row-frame alive as a product path beside binary-table chunks | Rust runtime transports no longer accept JSON row-frame chunks. Decide whether server/core should keep JSON row-frame as a test/debug format or remove it when binary/artifact paths cover all non-Rust clients |
