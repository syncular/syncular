...
| Console WebSocket first-message auth | `Accepted` | `packages/server/src/hono/console/routes.ts`, `packages/console/src/hooks/useLiveEvents.ts` | Browser WebSocket clients cannot send custom `Authorization` headers, and query-token auth would leak bearer tokens into URLs/logs | Can look like an auth fallback if undocumented | Keep as a platform capability; route middleware and tests must make the first-message auth path explicit |
| Service-worker postMessage wake delivery | `Accepted` | `packages/server/src/service-worker/index.ts` | Platform capability fallback when `BroadcastChannel` is unavailable or fails in a service-worker runtime | Can hide less direct wake delivery if untested | Keep as a browser/service-worker capability; tests cover BroadcastChannel delivery and client `postMessage` fallback |
...
| Redundant docs root-prefix redirect | `Temporary` | `apps/docs/next.config.mjs` | The public docs host moved to root-relative paths, but the explicitly requested cleanup needs `docs.syncular.dev/docs` to land on the canonical root instead of a 404 | Can mask stale external links that still include `/docs` | Remove after cached/external `/docs` links have aged out and public link scans stay clean |

## Recently Removed

| Item | Status | Removed From | Reason |
| --- | --- | --- | --- |
| `syncular` umbrella re-exports (`syncular`, `syncular/client`, `syncular/react`, `syncular/server*`, `syncular/console*`, `syncular/ui*`, `syncular/dialects/*`, and the other passthrough subpaths) | `Removed` | `packages/syncular/src/*` passthrough modules, `packages/syncular/package.json` exports map and `@syncular/*` dependencies, docs and package READMEs | The scoped `@syncular/*` packages are the canonical import surface. The `syncular` package is CLI-only (`npx syncular generate` / `npx syncular codegen install`); duplicate umbrella import aliases were deleted instead of maintained as a compatibility namespace |
| Per-driver dialect packages (`@syncular/dialect-better-sqlite3`, `@syncular/dialect-bun-sqlite`, `@syncular/dialect-d1`, `@syncular/dialect-libsql`, `@syncular/dialect-neon`, `@syncular/dialect-pglite`, `@syncular/dialect-sqlite3`) and `syncular/dialect-*` umbrella subpaths | `Removed` | `packages/dialect-*` package directories, `packages/syncular/src/dialect-*.ts`, package manifests, docs | Consolidated into `@syncular/server/<driver>` subpath exports; drivers remain optional peer dependencies so users install only the driver they import |
| Standalone 0.1-era client/server add-on packages (`@syncular/react`, `@syncular/client-tauri`, `@syncular/client-react-native`, `@syncular/client-crdt-adapters`, `@syncular/client-javascript-bindings`, `@syncular/dialects`, `@syncular/server-hono`, `@syncular/server-cloudflare`, `@syncular/server-service-worker`, `@syncular/server-dialect-sqlite`, `@syncular/server-dialect-postgres`, `@syncular/server-storage-filesystem`, `@syncular/server-storage-s3`, `@syncular/server-plugin-yjs`, `@syncular/relay`, `@syncular/transport-http`) | `Removed` | Deleted package directories under `packages/`, `plugins/`, and `rust/bindings/javascript`; workspace manifests; package docs | The public npm surface is consolidated into fewer packages with opt-in subpaths: `@syncular/client/*`, `@syncular/server/*`, and `@syncular/core/http`. No compatibility alias packages are retained because 0.1.0 has not been successfully published yet |
| Websocket JSON inline deltas | `Removed` | `packages/server/src/hono/ws.ts`, browser worker realtime contract/tests | Rust-first realtime protocol is binary sync-pack deltas or explicit pull-required wakeups. JSON row deltas were old product-protocol surface and bypassed the verified sync-pack contract |
...
| Server Hono legacy sync CORS shape | `Removed` | `packages/server/src/hono/routes.ts` | Hono-style `cors: origin` / `cors: { origin }` is the single sync route CORS contract |
...
| Old JS client product packages | `Removed` | `packages/client`, legacy React client implementation, client plugin packages, old JS client docs, demo app, legacy JS integration/runtime/perf suites | `@syncular/client` is now the Rust-owned browser package with TypeScript bindings and `@syncular/client/react` |
...
| `syncular/server-dialect-neon` umbrella alias | `Removed` | `packages/syncular/src/server-dialect-neon.ts`, `packages/syncular/package.json`, docs | Neon support is exposed through `@syncular/server/postgres`; the extra alias only preserved an unnecessary old import path |
...
| Service-worker legacy single-commit wake parsing | `Removed` | `packages/server/src/service-worker/index.ts`, service-worker tests | Wake resolution now reads current batched `push.commits` request/response shapes only. Legacy single `push.operations` / `push.status` handling was removed instead of carried as an old protocol branch |
...
| Transport per-call `onAuthError` callback | `Removed` | `SyncTransportOptions`, `@syncular/core/http` retry resolver/tests | `authLifecycle` is the single auth refresh contract. The legacy per-call callback was removed instead of taking precedence over the current lifecycle API |
...
| Explicit `json-v1` sync-pack negotiation | `Removed` | `packages/core/src/sync-packs.ts`, `packages/server/src/hono/routes.ts`, Rust protocol validation, protocol fixtures | Combined sync responses are binary sync-packs only. JSON HTTP error envelopes remain separate error responses, not a sync-pack encoding |
...
| Standalone managed-client constructor (`createSyncularClient`, `SyncularClient`, `SyncularManagedClient`, `CreateSyncularClientOptions`) | `Removed` | `packages/client/src/client.ts`, `@syncular/client` public exports, `@syncular/client/react` provider, generated TypeScript factory | `createSyncularDatabase` (and the generated `createSyncularAppDatabase`) now owns the sync lifecycle: it registers subscriptions, runs the initial sync, and starts realtime during open, exposes `start`/`stop`/`sync`/`on`/`getStatus`/presence/conflicts directly on the database, and stops sync on `close()`. The separate managed wrapper was removed instead of kept as a second init path; `SyncularClientLifecycle` remains exported for advanced control via `database.lifecycle` |
| Client `destroy()` teardown alias | `Removed` | `SyncularClientLike`, `@syncular/client` bridge/worker clients, `@syncular/client/react` (`destroyOnUnmount` → `closeOnUnmount`), platform bridge tests | `close()` is the single teardown method across database, worker, and bridge clients. The duplicate `destroy()` spelling was removed instead of carried as an alias |
