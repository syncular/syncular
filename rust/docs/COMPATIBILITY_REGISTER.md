...
| Per-driver client dialect packages (`@syncular/dialect-better-sqlite3`, `@syncular/dialect-bun-sqlite`, `@syncular/dialect-d1`, `@syncular/dialect-libsql`, `@syncular/dialect-neon`, `@syncular/dialect-pglite`, `@syncular/dialect-sqlite3`) and `syncular/dialect-*` umbrella subpaths | `Removed` | `packages/dialect-*` package directories, `packages/syncular/src/dialect-*.ts`, package manifests, docs | Consolidated into the single `@syncular/dialects` package with per-dialect subpath exports (`@syncular/dialects/<name>`, umbrella `syncular/dialects/<name>`); drivers are optional peer dependencies so users install only their driver |
...
| Standalone managed-client constructor (`createSyncularClient`, `SyncularClient`, `SyncularManagedClient`, `CreateSyncularClientOptions`) | `Removed` | `packages/client/src/client.ts`, `@syncular/client` public exports, `@syncular/react` provider, generated TypeScript factory | `createSyncularDatabase` (and the generated `createSyncularAppDatabase`) now owns the sync lifecycle: it registers subscriptions, runs the initial sync, and starts realtime during open, exposes `start`/`stop`/`sync`/`on`/`getStatus`/presence/conflicts directly on the database, and stops sync on `close()`. The separate managed wrapper was removed instead of kept as a second init path; `SyncularClientLifecycle` remains exported for advanced control via `database.lifecycle` |
| Client `destroy()` teardown alias | `Removed` | `SyncularClientLike`, `@syncular/client` bridge/worker clients, `@syncular/react` (`destroyOnUnmount` → `closeOnUnmount`), platform bridge tests | `close()` is the single teardown method across database, worker, and bridge clients. The duplicate `destroy()` spelling was removed instead of carried as an alias |
