...
export const SYNCULAR_ERROR_DEFINITIONS = {
  ...
  'sync.auth_lease_missing': {
    ...
    retryable: true,
    recommendedAction: 'refreshAuth',
    message: 'The queued commit is missing required auth lease provenance.',
  },
  'sync.auth_lease_invalid': {
    category: 'auth-required',
    retryable: true,
    recommendedAction: 'refreshAuth',
    message: 'The queued commit auth lease provenance is invalid.',
  },
  'sync.auth_lease_expired': {
    category: 'auth-required',
    retryable: true,
    recommendedAction: 'refreshAuth',
    message: 'The queued commit auth lease expired before replay.',
  },
  ...
  'sync.auth_lease_scope_mismatch': {
    ...
    message: 'The queued commit auth lease does not cover the pushed scopes.',
  },
  'sync.auth_lease_scope_revoked': {
    category: 'scope-revoked',
    retryable: false,
    recommendedAction: 'checkPermissions',
    message: 'The queued commit auth lease scope was revoked.',
  },
  'sync.auth_lease_business_rejected': {
    category: 'conflict',
    retryable: false,
    recommendedAction: 'resolveConflict',
    message: 'The queued commit auth lease was rejected by application policy.',
  },
  'sync.forbidden': {
    category: 'forbidden',
    retryable: false,
  ...
  'sync.empty_commit': {
    ...
    retryable: false,
    recommendedAction: 'fixRequest',
    message: 'The pushed commit has no operations.',
  },
  'sync.unknown_table': {
    category: 'schema-mismatch',
    retryable: false,
    recommendedAction: 'regenerateClient',
    message: 'The operation targets a table unknown to this server.',
  },
  'sync.unsupported_operation': {
    category: 'invalid-request',
    retryable: false,
    recommendedAction: 'fixRequest',
    message: 'The operation type is not supported for this table.',
  },
  'sync.row_missing': {
    category: 'not-found',
    retryable: false,
    recommendedAction: 'forceResync',
    message: 'The operation base row is missing on the server.',
  },
  ...
  'sync.constraint_violation': {
    ...
    retryable: false,
    recommendedAction: 'fixRequest',
    message: 'The operation violates a server-side data constraint.',
  },
  'sync.missing_scopes': {
    category: 'internal',
    retryable: false,
    recommendedAction: 'inspectServer',
    message: 'The server handler emitted a change without scope metadata.',
  },
  'sync.idempotency_cache_miss': {
    category: 'internal',
    retryable: true,
    recommendedAction: 'retryLater',
    message: 'The server could not read the cached idempotent push result.',
  },
