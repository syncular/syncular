...
export interface TopologySvgMediaProps {
  className?: string;
}

export function TopologySvgMedia({ className }: TopologySvgMediaProps) {
  const serverX = 350;
  const serverY = 90;
  const clientAX = 100;
  const clientAY = 90;
  ...
  return (
    ...
    >
      ...
      {/* Blob particle A -> Server (rectangular) */}
      <g opacity="0">
        <animateMotion
          dur="2.5s"
          repeatCount="indefinite"
      ...
      {/* Blob particle Server -> B (rectangular) */}
      <g opacity="0">
        <animateMotion
          dur="2.5s"
          begin="0.8s"
