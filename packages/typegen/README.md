...
| `naming` | no (default `"camel"`) | §5-of-implementation contract-queries casing: `"camel"` maps snake_case SQL names to camelCase in generated row types/params (projections lower with `AS` aliases so runtime keys match); `"preserve"` keeps SQL-truth names. Collisions/keyword hazards are generate-time errors. |
...
Two frontends feed one pipeline (query contract is the design of record):
