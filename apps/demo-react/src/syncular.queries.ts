...
// irVersion: 1
// irHash: sha256:e9b6c4c011d5a7a670e72d9bf8f1929457bd6b8efce11c44e6edf95212fddaf8
...
const listTodosStatements: (QueryRelationPlan & { bind: (params: ListTodosParams) => QueryValue[] })[] = [
  { sql: 'select id, list_id AS listId, title, done, position, updated_at_ms AS updatedAtMs, attachment from todos where todos.list_id = ? and 1 order by position, id', relations: [{"table":"todos","start":99,"end":104}], bind: (params) => [params.listId] },
  { sql: 'select id, list_id AS listId, title, done, position, updated_at_ms AS updatedAtMs, attachment from todos where todos.list_id = ? and ( title like \'%\' || ? || \'%\') order by position, id', relations: [{"table":"todos","start":99,"end":104}], bind: (params) => [params.listId, params.q ?? null] },
];
...
export const listTodosQuery: NamedQuery<ListTodosRow, ListTodosParams> = {
  id: 'sha256:e9b6c4c011d5a7a670e72d9bf8f1929457bd6b8efce11c44e6edf95212fddaf8/listTodos',
  hasParams: true,
