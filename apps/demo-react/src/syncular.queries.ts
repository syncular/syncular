...
// irVersion: 1
// irHash: sha256:e0befc829c127eb1ff397607e588b2176218ad4a04dc81da9e4e46bbd56d664c
...
function listTodosValidate(raw?: ListTodosParams): ListTodosParams {
  ...
  const allowed = new Set(['listId', 'q']);
  ...
  if (params.q !== undefined && !(typeof params.q === 'string')) throw new SyqlInputError('SYQL_RUNTIME_INVALID_INPUT', 'listTodos: invalid optional input q');
  return params;
}
...
const listTodosStatements: (QueryRelationPlan & { bind: (params: ListTodosParams) => QueryValue[] })[] = [
  { sql: 'select id, list_id AS "listId", title, done, position, updated_at_ms AS "updatedAtMs", attachment from todos where todos.list_id = ? and 1 order by position, id', relations: [{"table":"todos","start":103,"end":108}], bind: (params) => [params.listId] },
  { sql: 'select id, list_id AS "listId", title, done, position, updated_at_ms AS "updatedAtMs", attachment from todos where todos.list_id = ? and ( title like \'%\' || ? || \'%\') order by position, id', relations: [{"table":"todos","start":103,"end":108}], bind: (params) => [params.listId, params.q ?? null] },
];
function listTodosSelect(raw: ListTodosParams): { sql: string; bind: QueryValue[] } {
  const params = listTodosValidate(raw);
  let mask = 0;
  if (params.q !== undefined) mask |= 1;
...
export const listTodosQuery: NamedQuery<ListTodosRow, ListTodosParams> = {
  id: 'sha256:e0befc829c127eb1ff397607e588b2176218ad4a04dc81da9e4e46bbd56d664c/listTodos',
  hasParams: true,
