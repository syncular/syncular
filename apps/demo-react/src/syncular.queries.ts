...
// irVersion: 1
// irHash: sha256:b984841d9d39ab258ffddac9151f500fd01da6a1442ecb79deca99f83d3979f5
...
const listTodosStatements: { sql: string; bind: (params: ListTodosParams) => QueryValue[] }[] = [
  { sql: 'select id, list_id AS listId, title, done, position, updated_at_ms AS updatedAtMs, attachment from todos where (todos.list_id = ?) and 1 order by position, id', bind: (params) => [params.listId] },
  { sql: 'select id, list_id AS listId, title, done, position, updated_at_ms AS updatedAtMs, attachment from todos where (todos.list_id = ?) and ( title like \'%\' || ? || \'%\' ) order by position, id', bind: (params) => [params.listId, params.q ?? null] },
];
...
export const listTodosQuery: NamedQuery<ListTodosRow, ListTodosParams> = {
  id: 'sha256:b984841d9d39ab258ffddac9151f500fd01da6a1442ecb79deca99f83d3979f5/listTodos',
  hasParams: true,
