...
function AlicePanel({
  ...
  const transport = useMemo(
    () =>
      createWebSocketTransport({
        ...
        getHeaders: () => ({ 'x-user-id': actorId }),
        getRealtimeParams: () => ({ userId: actorId }),
      }),
    []
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const database = createSqliteClient('demo-keyshare-v3.sqlite');
    ...
    (async () => {
      try {
        const database = await createPgliteClient(
          'idb://sync-demo-keyshare-pglite-v3'
        );
        await migrateClientDb(database);
        setDb(database);
      } catch (err) {
