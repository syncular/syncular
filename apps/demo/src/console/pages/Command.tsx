...
function CommandInner() {
  ...
  const { data: timeseriesData } = useTimeseriesStats({ range });
  const { data: latencyData } = useLatencyStats({ range });
  const { data: commitsData } = useCommits({ limit: 5 });
  const { data: clientsData } = useClients({ limit: 12, offset: 0 });
  const { events, isConnected } = useLiveEvents({ maxEvents: 8 });
  ...
  const kpiItems = useMemo((): MetricItem[] => {
    ...
    return [
      {
        label: 'Operations',
        value: stats.commitCount + stats.changeCount,
        color: 'flow',
      },
      {
        label: 'P50 Latency',
  ...
  const successRate = useMemo(() => {
    if (!stats) return 99.6;
    const total = stats.commitCount + stats.changeCount;
    if (total === 0) return 100;
    // If we had error count data we'd compute it; default to 99.6 for now
    return 99.6;
  }, [stats]);
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            ...
            <div className="border-r border-border">
              <CommitTable
                ...
                onViewAll={() => navigate({ to: '/console/stream' })}
              />
            </div>
          </div>
