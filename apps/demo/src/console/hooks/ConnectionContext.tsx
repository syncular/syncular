...
import { getDefaultDemoConnectionConfig } from '../lib/default-connection';
...
export function ConnectionProvider({ children }: { children: ReactNode }) {
  ...
  // Resolve initial config: URL params -> saved config -> demo defaults
  useEffect(() => {
    ...
    if (server?.trim() && token?.trim()) {
      setConfigStorage({ serverUrl: server.trim(), token: token.trim() });
      // Clean up URL
    ...
    const defaultConfig = getDefaultDemoConnectionConfig();
    if (defaultConfig) {
