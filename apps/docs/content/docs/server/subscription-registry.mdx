---
title: Shape Handlers & Scopes
description: Configure data access through shape handlers
---

Shape handlers define how data syncs and what scopes users can access. Each handler has `resolveScopes` which controls authorization.
...
Clients request subscriptions by shape and scopes:

```typescript
<SyncProvider
  subscriptions={[
    { id: 'my-tasks', shape: 'tasks', scopes: { user_id: userId } },
    { id: 'project-1', shape: 'tasks', scopes: { project_id: 'p1' } },
  ]}
  ...
/>
```
...
1. Client requests subscription with `shape` and `scopes`
2. Server calls `resolveScopes` on the shape handler
