...
- Familiarity with [How Sync Works](/learn/architecture)
...
If your runtime uses Neon instead of `pg`, install `@syncular/dialects` (plus
the `kysely-neon` and `@neondatabase/serverless` drivers) and use
`createNeonServerDialect()` below.
...
See [Table Handlers](/server/table-handlers) for advanced patterns and
[Scopes And Auth](/server/scopes-and-auth) for authorization strategies.
...
See [Realtime WebSocket](/server/realtime-websocket) for multi-instance broadcasting and client setup.
...
See [Operations Guide](/operate/operations-setup) for console API details.
...
- [JavaScript](/clients/javascript) or [Rust](/clients/rust) - set up a client
- [Table Handlers](/server/table-handlers) - deep dive into handlers
- [Scopes And Auth](/server/scopes-and-auth) - authorization patterns
- [Deployment](/operate/deployment) - production deployment
