...
- A working Syncular setup ([Server Setup](/docs/guides/server-setup), [Client Setup](/docs/guides/client-setup))
...
Syncular uses **snapshot chunks**: the server encodes rows as gzip-compressed NDJSON, caches them, and returns chunk references in the pull response. The client then downloads chunks separately via `GET /sync/snapshot-chunks/:chunkId`.
...
These are server-enforced upper bounds. The client can request smaller values.
...
- **Keep `maxSnapshotPages` reasonable** (5-10) to avoid blocking a single pull for too long
...
For very large deployments, the server supports external chunk storage (S3, R2, etc.):

```typescript
const chunk = await readSnapshotChunk(db, chunkId, {
  chunkStorage: {
    async readChunk(id: string): Promise<Uint8Array | null> {
      // Read from S3, R2, etc.
      return await s3.getObject(id);
    },
  },
});
```
...
subscriptions={[
  { shape: 'notes', scopes: { user_id: userId } },
]}

// Avoid: overly broad subscription (pulls all users' notes)
subscriptions={[
  { shape: 'notes', scopes: {} },
]}
```
...
subscriptions={[
  { shape: 'notes', scopes: { user_id: userId } },
  { shape: 'notes', scopes: { workspace_id: workspaceId } },
  { shape: 'tasks', scopes: { workspace_id: workspaceId } },
]}
```
...
- [Schema Migrations](/docs/guides/migrations) - Evolve your schema
- [Realtime](/docs/guides/realtime) - Configure WebSocket wake-ups
