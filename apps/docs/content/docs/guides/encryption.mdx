...
- A working Syncular client setup ([Client Setup](/docs/guides/client-setup))
...
interface FieldEncryptionRule {
  /** Scope identifier (typically the subscription shape name) */
  scope: string;
...
function App() {
  return (
    <SyncProvider
      ...
      clientId={clientId}
      plugins={[encryptionPlugin]}
      subscriptions={[{ shape: 'notes', scopes: { user_id: userId } }]}
    >
      <Notes />
    </SyncProvider>
  );
...
- [Authentication](/docs/guides/auth) - Secure your transport layer
- [Data Modeling](/docs/guides/data-modeling) - Design schemas for encrypted data
