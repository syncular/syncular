...
// Electron renderer (SQLite in main process over IPC)
// import { createElectronSqliteDbFromWindow } from '@syncular/dialect-electron-sqlite';
// const db = createElectronSqliteDbFromWindow<DB>({ bridgeKey: 'app' });
//
// Electron main process can adapt any SQLite Kysely dialect:
// createElectronSqliteBridgeFromDialect({ dialect: createBetterSqlite3Dialect({ path: './app.sqlite' }) })
```
