...
Syncular's sync engine provides structured error handling with automatic retries, error event callbacks, and React hooks for displaying sync status to users.
...
- A working Syncular React setup ([Client Setup](/docs/guides/client-setup))
...
| `SYNC_ERROR` | Server returned an error, migration failed, or sync logic error |
...
After each successful sync, the retry counter resets to 0.
...
- [Realtime](/docs/guides/realtime) - WebSocket wake-ups for instant updates
- [Operations](/docs/guides/operations) - Mutations and the outbox
