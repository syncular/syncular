...
PostgreSQL note: `numeric`/`decimal` columns are generated as `string` by default (matching common pg driver return types). If your runtime config parses them to `number`, override with `resolveType`.
