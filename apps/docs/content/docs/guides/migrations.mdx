...
As your application evolves, you need to add tables, rename columns, and change data shapes. Syncular supports schema migrations on both the server and client, with a `schemaVersion` field in push requests to ensure the server can handle operations from clients on different schema versions.
...
- A working Syncular setup with server and client ([Server Setup](/docs/guides/server-setup), [Client Setup](/docs/guides/client-setup))
...
- [Table Handlers](/docs/guides/table-handlers) - Define server-side table handlers
- [Data Modeling](/docs/guides/data-modeling) - Design schemas for sync
- [Performance & Scaling](/docs/guides/performance) - Optimize for large datasets
