...
- Familiarity with [Scopes](/docs/introduction/scopes) and [Server Setup](/docs/guides/server-setup)
...
const handlers = new ClientTableRegistry()
  .register(tasksClientHandler)
  .register(projectsClientHandler);

<SyncProvider
  db={db}
  transport={transport}
  handlers={handlers}  // Pass custom handlers
  ...
/>
```
...
- [Authorization](/docs/guides/auth) — Auth patterns with scopes
