...
- How to verify URL wiring, shape names, and scope alignment
...
If the client is configured with `baseUrl: '/api/sync'`, requests will go to `/api/sync/sync/pull` and fail.

### 2) Verify your shape names

- Server: `createServerHandler({ table: 'tasks', ... })` registers a shape named `tasks`
- Client subscription: `{ shape: 'tasks', ... }` must match exactly
...
- In browsers, you cannot send custom headers with WebSocket:
  ...
  - Use a non-sensitive query param token, or
  - Use a first-message auth token if your server supports it.

See: [Realtime](/docs/guides/realtime).
...
- Use snapshot chunks (NDJSON+gzip) and ensure the `/sync/snapshot-chunks/:chunkId` route is reachable.
- Index your scope columns and primary keys appropriately (especially on the server DB).

## Next steps

- [Data Modeling with Scopes](/docs/guides/data-modeling)
