...
const proxyTableRegistry = new ProxyTableRegistry()
  .register({
    table: 'tasks',
    primaryKey: 'id',
...
```typescript
const ws = new WebSocket('wss://api.example.com/proxy');
...
- [Operations Guide](/docs/guides/operations) - Console and monitoring
- [Server Setup](/docs/guides/server-setup) - Full server configuration
- [Auth Guide](/docs/guides/auth) - Authentication patterns
