...
## Scripted ack loss and reconnect drops

Use a `plan` when you need deterministic pass/fail sequences or when the
failure should happen after the server already responded:
...
const wrapped = withFaults(client.transport, {
  plan: [
    { operation: 'push', action: 'pass' },
    {
      operation: 'push',
      phase: 'after',
      failWith: new Error('simulated ack loss'),
    },
  ],
});
...
await client.push(); // succeeds
await client.push(); // server applies commit, caller sees an error
```
