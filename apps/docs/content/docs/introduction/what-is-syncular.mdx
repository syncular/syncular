...
Syncular is a **sync engine** for building offline-first applications. It handles the hard parts of synchronization — local storage, incremental sync, conflict detection, authorization, and real-time updates — so you can focus on building your app.
...
| **Shapes** | Server-side definitions of how each table syncs. Clients subscribe by shape name. |
| **Subscriptions** | Client requests for data: a shape + scope values. Example: `{ shape: 'tasks', scopes: { project_id: 'A' } }` |
