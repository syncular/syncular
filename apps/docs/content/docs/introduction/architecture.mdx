...
For large datasets, snapshots are returned as **compressed chunks** (gzip + ndjson). The client fetches each chunk separately via `GET /sync/snapshot-chunks/:chunkId` and streams rows into SQLite.
