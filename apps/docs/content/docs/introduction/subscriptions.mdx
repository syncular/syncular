...
Subscriptions define what data a client wants to sync. Each subscription pairs a **shape** (a server-defined table handler) with **scope values** that filter the data.

## What you'll learn

- The structure of a subscription and what each field means
- Why shapes are server-defined (not arbitrary client queries)
...
Syncular uses **pre-defined shapes** instead. The client requests data by shape name and scope values; the server validates access via `resolveScopes`. For details on how scope validation works, see [Scopes](/docs/introduction/scopes).
...
```typescript
{
  ...
  shape: 'tasks',              // Name of a server-defined shape handler
...
{ id: `project-${projectId}`, shape: 'tasks', scopes: { project_id: projectId } }

// Bad: random or unstable IDs cause re-bootstraps on every render
{ id: crypto.randomUUID(), shape: 'tasks', scopes: { user_id: userId } }
{ id: `tasks-${Date.now()}`, shape: 'tasks', scopes: { user_id: userId } }
```
...
<SyncProvider
  ...
  subscriptions={[
    { id: 'my-tasks', shape: 'tasks', scopes: { user_id: userId } },
    { id: 'project-a', shape: 'tasks', scopes: { project_id: 'a' } },
  ]}
>
```
...
function App() {
  ...
  // Subscriptions update whenever selectedProjects changes
  const subscriptions = [
...
```tsx
// When userId changes, 'my-tasks' ID stays the same but the scopes change.
// The system detects scope changes and re-bootstraps.
const subscriptions = [
  { id: `tasks-${userId}`, shape: 'tasks', scopes: { user_id: userId } },
];
```
