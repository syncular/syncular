...
2. **Push** -- Client POSTs to `/sync/push` with `clientId`, `clientCommitId`, and `operations`.
...
1. **Request** -- Client POSTs `/sync/pull` with an array of subscriptions. Each subscription has a `shape`, `scopes`, and a `cursor` (position in the commit log).
