...
| `checksum` | Behavior |
|-----------|----------|
| `'deterministic'` (default) | Replays the migration into a scratch in-memory DB that matches the current runtime family and hashes the SQL trace that actually executed. |
| `'disabled'` | Skips checksum creation and checksum mismatch checks for that migration. Use this for browser/service-worker runtimes or other cases where Syncular cannot reliably create the matching scratch DB. |
...
Syncular does not hash `migration.up.toString()`. For `checksum: 'deterministic'`, it replays the migration into a scratch in-memory database and hashes the SQL statements that actually ran. That keeps checksum validation stable across bundling/minification while still validating that the migration can execute against a real database.
