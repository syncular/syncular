...
- Client-side migration with `createSyncularV2Database`
...
Use `createSyncularV2Database` from `@syncular/client` to create these tables. It is idempotent and safe to call on every app startup:

```typescript
import { createSyncularV2Database } from '@syncular/client';

async function initializeDatabase(db: Kysely<MyDb>) {
  // Create sync infrastructure tables
  await createSyncularV2Database(db);
...
```typescript
import { withSyncularV2SchemaWrites, createSyncularV2Database } from '@syncular/client';
...
async function migrateClientDb(db: Kysely<MyDb>) {
  ...
  await runMigrations({
    ...
    beforeReset: async (db) => {
      ...
      await withSyncularV2SchemaWrites(db);
      await createSyncularV2Database(db);
    },
  });
}
...
To fully reset sync state (e.g., when a user logs out), use `withSyncularV2SchemaWrites`:

```typescript
import { withSyncularV2SchemaWrites } from '@syncular/client';
...
This removes `sync_conflicts`, `sync_outbox_commits`, and `sync_subscription_state`. The next app startup will recreate them fresh via `createSyncularV2Database`.
