...
- Familiarity with [Scopes](/docs/concepts/scopes) and [Server Setup](/docs/build/server-setup)
  ...
  authorize: async (ctx, op) => {
    if (op.op === 'upsert') {
      ...
      if (userId !== ctx.actorId) {
        return { error: 'Cannot create tasks for other users', code: 'sync.forbidden' };
      }
    }
    return true;
  },
