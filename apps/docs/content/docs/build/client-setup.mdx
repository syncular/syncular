...
- React 18+
- Familiarity with [How Sync Works](/docs/concepts/architecture)

---

The client runs a **local database first** (SQLite or PGlite via Kysely), then syncs with your server in the background.

This guide shows the recommended setup for React + `@syncular/client-react`. For non-React usage, see the [JavaScript SDK Overview](/docs/reference/client-sdk/javascript/overview).
...
The client scope definitions must describe columns that exist in the local
table. Syncular uses those mappings to clear only the rows for a removed,
revoked, or re-bootstrapped subscription. If the server uses
`user:{user_id}` but your local table stores that value in `owner_user_id`, map
it explicitly:
...
Rows sent by the server should include the local scope column if the default
handler needs to write it locally. If you intentionally hide that metadata from
row payloads, provide custom `applySnapshot` and `applyChange` handlers that
write whatever local metadata your schema requires.

Only skip local scope columns when you provide custom `clearAll` and
`onSnapshotStart` handlers. Otherwise Syncular cannot safely distinguish
"clear this scope" from "clear the whole table".
...
For Expo / React Native, prefer the runtime preset:
...
That preset enables byte-oriented snapshot fetches, gzip fallback, and
per-subscription bootstrap commits for Hermes / Expo runtimes.
...
For mobile clients, the known-good path is:

- `@syncular/dialect-expo-sqlite` or `@syncular/dialect-react-native-nitro-sqlite`
- `createReactNativeHttpTransport(...)`
- `bootstrapPhase` on heavy subscriptions so startup is not blocked by relation
  tables or large secondary datasets
- `useSyncBootstrapState()` for startup gating instead of querying sync tables
  manually

If mobile startup stays blank for too long, the first thing to check is whether
one large phase-0 subscription is still bootstrapping.
...
For multi-workspace apps, include the runtime/workspace identity in that key and
memoize the identity object you pass to the provider:
  ...
  const [db, dbError] = useCachedAsyncValue(
    () =>
      dbInitRegistry.run('primary-db', async () => {
        const database = createDatabase<AppDb>({
          dialect: createWaSqliteDialect({ fileName: 'app.sqlite' }),
          family: 'sqlite',
        });
...
Called when a subscription is removed or revoked. The default clears by the
handler's scope mappings when `ctx.scopes` is non-empty. It clears the whole
table only for global subscriptions with empty scopes.
