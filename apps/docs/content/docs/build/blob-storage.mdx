...
sequenceDiagram
  ...
  Client->>Server: Initiate upload
  Server-->>Client: Presigned upload URL
  Client->>Store: Upload blob bytes
  Client->>Server: Mark upload complete
  Client->>Server: Include blob ref in sync data
`}
  caption="Blob bytes bypass the sync payload. Syncular coordinates metadata, auth, and completion while the actual upload goes directly to the storage backend."
/>
...
const blobRoutes = createBlobRoutes({
  ...
  tokenSigner,
  db,
  // Required: enforce tenant/ownership access control
...
```typescript
import type { ClientBlobStorage } from '@syncular/client-plugin-blob';
...
For complete API documentation, see the [API Reference](/docs/reference/api).
