...
Setting `maxRetries` to `0` disables automatic retries. You can still trigger manual syncs via `useSyncularV2ManagedClient().sync()`.
...
function useErrorLogger() {
  const { state, sync } = useSyncularV2ManagedClient();
...
function OfflineBanner() {
  const status = useSyncStatus();
  const { reconnect, sync } = useSyncularV2ManagedClient();
