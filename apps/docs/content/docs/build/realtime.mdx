...
HTTP CORS and WebSocket origins are separate checks. Configuring Hono CORS for
`/api/*` does not allow cross-origin WebSocket upgrades by itself. If your
browser, dev server, or Tauri app connects from another origin, add that origin
to `websocket.allowedOrigins`. Use `'*'` only for local development or trusted
private deployments.
...
The built-in WebSocket transport keeps reconnect attempts single-flight: a newer
connect/reconnect attempt closes the stale socket before opening another one,
and stale socket callbacks are ignored. If you write a custom realtime
transport, use the same pattern with a generation token or abort signal so an
older connection attempt cannot close a newer socket.
