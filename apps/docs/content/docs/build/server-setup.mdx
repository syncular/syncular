...
See [Table Handlers](/docs/reference/server/table-handlers-reference) for advanced patterns and [Auth](/docs/concepts/scopes) for authorization strategies.
...
See [Real-time Guide](/docs/reference/server/realtime-bridge) for multi-instance broadcasting and client setup.
...
Use an explicit `console.corsOrigins` allowlist in production rather than relying on defaults.
...
createSyncServer({
  ...
  sync: {
    ...
    requestPayloadSnapshots: {
      enabled: true,                  // Opt in to request/response payload snapshots (default: false)
      maxBytes: 16384,                // Keep snapshots with aggressive truncation (16 KiB)
    },
  },
});
```
...
- [Client Setup](/docs/rust-client/quick-start) — set up the React client
- [Table Handlers](/docs/reference/server/table-handlers-reference) — deep dive into handlers
- [Auth](/docs/concepts/scopes) — authorization patterns
