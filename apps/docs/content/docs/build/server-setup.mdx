...
<Mermaid
  chart={`
flowchart LR
  Requested["1. Client subscription<br>{ table: tasks, scopes: { user_id: '123' } }"] --> Allowed["2. resolveScopes(ctx)<br>{ user_id: ['123'] }"]
  Allowed --> Effective["3. Effective scopes<br>requested ∩ allowed"]
  Effective --> Pull["4. Pull returns matching rows only"]
  Push["5. Push operation"] --> Extract["Scopes extracted from row data"]
  Extract --> Tagged["Tagged on emitted changes"]
`}
