...
The sync engine provides fine-grained control over the sync process. These are low-level building blocks -- most applications should use the [`Client` class](/docs/sdk/javascript/overview) instead.
...
Create or update a subscription's state. Use this when manually managing subscription cursors, for example after a full reset or when migrating subscription shapes.
...
- [Mutations](/docs/sdk/javascript/mutations) - High-level mutation API
