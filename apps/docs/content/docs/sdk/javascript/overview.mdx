...
```typescript
import { Client, ClientTableRegistry } from '@syncular/client';
import { createHttpTransport } from '@syncular/transport-http';

// Create transport
const transport = createHttpTransport({
  // baseUrl is the prefix *before* `/sync/*` (e.g. /api → /api/sync/pull)
  baseUrl: '/api',
  getHeaders: () => ({ Authorization: `Bearer ${token}` }),
});

// Register client table handlers
const tableHandlers = new ClientTableRegistry()
...
const client = new Client({
  ...
  actorId: 'user-456',
  subscriptions: [
    { id: 'my-tasks', shape: 'tasks', scopes: {} },
  ],
});
...
interface ClientOptions<DB> {
  ...
  tableHandlers: ClientTableRegistry;
  clientId: string;
  actorId: string;
  subscriptions: Array<{
    id: string;
...
See the [Blob Storage Guide](/docs/guides/blob-storage) for adapter examples.
...
const result = await syncOnce(db, transport, shapes, {
  clientId: 'device-123',
  actorId: 'user-456',
  subscriptions: [...],
});
...
const engine = new SyncEngine({ db, transport, shapes, ... });
await engine.start();
...
- [Client Setup Guide](/docs/guides/client-setup) - Detailed setup instructions
- [Mutations](/docs/sdk/javascript/mutations) - Making changes
