...
| `shapes` | `ClientTableRegistry` | Yes | Client table/shape handlers |
| `actorId` | `string \| null` | No | User ID (null disables sync) |
| `clientId` | `string \| null` | No | Device/installation ID |
| `subscriptions` | `Array` | No | Data subscriptions |
...
```tsx
<SyncProvider
  actorId={isLoggedIn ? userId : null}
  ...
/>
```
