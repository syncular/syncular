...
- Client-side migration with `createSyncularDatabase`
...
- A working Syncular setup with server and client ([Server Setup](/docs/server/setup-with-hono), [Client Setup](/docs/client/quick-start))
...
Use `createSyncularDatabase` from `@syncular/client` to create these tables. It is idempotent and safe to call on every app startup:
...
async function initializeDatabase(db: Kysely<MyDb>) {
  ...
  await createSyncularDatabase(db);
...
```typescript
import { withSyncularSchemaWrites, createSyncularDatabase } from '@syncular/client';
import { createMigrationTrackingTableName, runMigrations } from '@syncular/migrations';
import { clientMigrations } from './client.migrations';
import { migrationChecksums } from './client.migrations.checksums.generated';
...
async function migrateClientDb(db: Kysely<MyDb>) {
  ...
  await runMigrations({
    ...
    trackingTable: createMigrationTrackingTableName(['notes', 'client']),
    onChecksumMismatch: 'reset',
    beforeReset: async (db) => {
      await db.schema.dropTable('notes').ifExists().execute();
      await withSyncularSchemaWrites(db);
      await createSyncularDatabase(db);
    },
  });
}
...
Keep the migration definitions themselves in a separate source module so type generation and checksum generation do not depend on their own generated artifacts:

```typescript
// client.migrations.ts
import { defineMigrations } from '@syncular/migrations';

export const clientMigrations = defineMigrations<MyDb>({
  v1: {
    up: async (db) => {
      await db.schema
        .createTable('notes')
  ...
  v2: {
    ...
    down: async (db) => {
      await db.schema.alterTable('notes').dropColumn('pinned').execute();
    },
  },
});
```

If you need a custom tracking table, prefer
`createMigrationTrackingTableName(...)` over hand-written string literals. It
normalizes names to lowercase snake_case and keeps the `migration_state`
suffix consistent.

#### Checksum modes

Each migration can opt into one of two checksum modes:

| `checksum` | Behavior |
|-----------|----------|
| `'deterministic'` (default) | Requires a generated checksum manifest entry for that migration. At runtime, Syncular compares the stored checksum against the generated SQL-trace checksum. |
| `'disabled'` | Skips checksum creation and checksum mismatch checks for that migration. Use this when build-time replay is not safe or meaningful. |

For deterministic migrations, `runMigrations` detects when the emitted SQL trace changes after a version has already been applied. You control what happens next:
...
#### Generated SQL-trace checksums

Syncular does not hash `migration.up.toString()`. Instead, `@syncular/typegen` replays deterministic migrations into an in-memory database during generation time and writes a checksum manifest next to your migration file. That keeps browser and React Native bundles clean while still validating the SQL that actually runs.
...
To fully reset sync state (e.g., when a user logs out), use `withSyncularSchemaWrites`:

```typescript
import { withSyncularSchemaWrites } from '@syncular/client';
...
This removes `sync_conflicts`, `sync_outbox_commits`, and `sync_subscription_state`. The next app startup will recreate them fresh via `createSyncularDatabase`.
...
`@syncular/typegen` introspects your migrations and generates TypeScript interfaces for your database. It can also generate checksum manifests for deterministic migration validation:
...
await generateTypes({
  ...
  extendsgenerated app database type: true, // Include generated app database type base tables
  syncularImportType: 'umbrella', // Optional. Default: 'scoped'
  includeVersionHistory: false, // Only generate types for the latest schema
});

await generateMigrationChecksums({
  migrations,
  output: './src/migrations.checksums.generated.ts',
});
```
...
const result = await generateTypes({
  migrations,
  output: './src/db.generated.ts',
  extendsgenerated app database type: true,
  syncularImportType: 'umbrella',
});

const checksums = await generateMigrationChecksums({
  migrations,
  output: './src/migrations.checksums.generated.ts',
});
...
console.log(`Generated ${checksums.checksumCount} migration checksums`);
```
...
import type { generated app database type } from '@syncular/client';
...
export interface DB extends generated app database type {
  notes: NotesTable;
}
```
...
Use `codecs` to define a column once and drive generated TypeScript types plus runtime DB conversion:
...
```typescript
import { createColumnCodecsPlugin, createDatabase } from '@syncular/core';
import { createBunSqliteDialect } from '@syncular/dialect-bun-sqlite';

const baseDb = createDatabase<DB>({
...
Use one conversion path per query. If Syncular handlers/mutations already apply `codecs`, keep a separate plugin-wrapped DB instance for app-level ad-hoc queries to avoid double conversion on the same statement.
...
- [Table Handlers](/docs/reference/server/table-handlers-reference) - Define server-side table handlers
- [Data Modeling](/docs/features/data-modeling) - Design schemas for sync
- [Performance & Scaling](/docs/operate/performance) - Optimize for large datasets
