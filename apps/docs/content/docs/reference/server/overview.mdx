...
const blobRoutes = createBlobRoutes({
  ...
  canAccessBlob: async ({ actorId, hash, partitionId }) => {
    // Enforce your tenant/ownership model before issuing download URLs.
    return true;
  },
});
...
- [Table Handlers Reference](/docs/reference/server/table-handlers-reference) - Define table handlers
- [Table Handlers & Scopes](/docs/reference/server/subscription-registry) - Configure scopes
