...
flowchart TB
  ...
  BBroadcaster <--> Bus
`}
  caption="Each server instance manages its own WebSocket clients. A shared broadcaster fan-outs wake-up events so every instance can notify the right connections."
/>
