...
| `allowedOrigins` | same-origin only | Browser websocket origin allowlist (`'*'` disables the guard) |
...
`GET /snapshot-chunks/:chunkId` is authenticated and scope-checked. The built-in HTTP transport sends the required `X-Syncular-Snapshot-Scopes` header automatically. If you call the chunk route yourself, send the same scope values that came back on the pull subscription or the route will reject the request.

## Error Responses

| Error Code | HTTP Status | Description |
|------------|-------------|-------------|
| `sync.auth_required` | 401 | Authentication failed |
| `sync.forbidden` | 403 | Not authorized for resource |
| `sync.invalid_request` | 400 | Malformed request |
| `sync.invalid_subscription` | 400 | Unknown table |
| `sync.too_many_operations` | 400 | Exceeded max operations |
...
- [Dialects](/docs/reference/server/dialects) - Database dialect configuration
- [Realtime Bridge](/docs/reference/server/realtime-bridge) - Multi-instance realtime
