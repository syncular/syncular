...
```typescript
import { createNeonServerDialect } from '@syncular/server/postgres';
...
All runtime dialect helpers ship as `@syncular/server/<driver>` subpaths
(for example `import { createPgliteDialect } from '@syncular/server/pglite'`).
Database drivers are optional peer dependencies — install `@syncular/server`
plus the driver for the dialect you use (e.g. `better-sqlite3`, `libsql`,
`kysely-d1`, `kysely-bun-sqlite`, `kysely-neon` + `@neondatabase/serverless`,
`kysely-pglite-dialect` + `@electric-sql/pglite`, or
`kysely-generic-sqlite` + `sqlite3`).
Use `createDatabase` from `@syncular/core` to construct `Kysely<T>` once, centrally.

### SQLite-compatible (use with `@syncular/server/sqlite`)

| Import | Runtime | Example |
|---------|---------|---------|
| `@syncular/server/better-sqlite3` | Node.js | `createDatabase({ dialect: createBetterSqlite3Dialect({ path: './data.db' }), family: 'sqlite' })` |
| `@syncular/server/sqlite3` | Node.js | `createDatabase({ dialect: createSqlite3Dialect({ path: './data.db' }), family: 'sqlite' })` |
| `@syncular/server/bun-sqlite` | Bun | `createDatabase({ dialect: createBunSqliteDialect({ path: './data.db' }), family: 'sqlite' })` |
| `@syncular/server/libsql` | Node.js / Edge | `createDatabase({ dialect: createLibsqlDialect({ url: 'libsql://...', authToken: '...' }), family: 'sqlite' })` |
| `@syncular/server/d1` | Cloudflare Workers | `createDatabase({ dialect: createD1Dialect(env.DB), family: 'sqlite' })` |
...
| `@syncular/server/neon` | Serverless / Edge | `createDatabase({ dialect: createNeonDialect({ connectionString: '...' }), family: 'postgres' })` |
| `@syncular/server/pglite` | Browser | `createDatabase({ dialect: createPgliteDialect({ dataDir: 'idb://app' }), family: 'postgres' })` |

For a Syncular server on Neon, the intended pairing is:

- runtime dialect: `@syncular/server/neon`
- server dialect: `createNeonServerDialect()`

## Migrations

Run sync infrastructure setup using the dialect from a deploy or operator
step:
...
- [Realtime WebSocket](/server/realtime-websocket) - Multi-instance realtime
- [Server Getting Started](/server/getting-started) - Complete setup guide
