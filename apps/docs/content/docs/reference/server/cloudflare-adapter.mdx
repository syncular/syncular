...
```bash
bun add @syncular/server kysely-d1 hono
```

For Postgres (Neon) instead of D1:

```bash
bun add @syncular/server kysely-neon @neondatabase/serverless
```
...
import { createSqliteServerDialect, ensureSyncSchema } from '@syncular/server/sqlite';
...
Cloudflare Durable Object realtime uses the same Syncular WebSocket origin guard as other Hono deployments. HTTP CORS and WebSocket origins are separate: configuring `routes.cors`, Hono CORS middleware, or Worker CORS responses does not allow cross-origin WebSocket upgrades by itself. Add every browser, local dev, and Tauri origin to `routes.websocket.allowedOrigins`; use `'*'` only for local development or trusted private deployments.
...
import { createPostgresServerDialect, ensureSyncSchema } from '@syncular/server/postgres';

export default createSyncWorker<Env>(async (app, env) => {
  const db = createDatabase({
    dialect: createNeonDialect({ connectionString: env.DATABASE_URL }),
    family: 'postgres',
  });
...
export default createSyncWorker<Env>(async (app, env) => {
  ...
  const blobRoutes = createBlobRoutes({
    ...
    canAccessBlob: async ({ actorId, hash, partitionId }) => {
      ...
      return checkBlobAccess({ actorId, hash, partitionId });
    },
  });
...
- [Hono Adapter](/reference/server/hono-adapter) - Hono route configuration
- [Dialects](/reference/server/dialects) - Database dialect options
- [Realtime](/server/realtime-websocket) - Client-side realtime setup
