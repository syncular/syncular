...
```bash
bun add @syncular/server-cloudflare @syncular/server-hono @syncular/dialects kysely-d1 @syncular/server-dialect-sqlite hono
```

For Postgres (Neon) instead of D1:

```bash
bun add @syncular/dialects kysely-neon @neondatabase/serverless @syncular/server-dialect-postgres
```
