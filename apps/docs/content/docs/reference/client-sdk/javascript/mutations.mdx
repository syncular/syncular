...
<Mermaid
  chart={`
flowchart TB
  Mutation["client.mutations.tasks.insert(...)"] --> Local["1. Local DB write (optimistic)"]
  Local --> Outbox["2. Outbox commit enqueued"]
  Outbox --> Push["3. Push to server"]
  Push --> Applied["4a. Applied and acked"]
  Push --> Conflict["4b. Conflict stored for resolution"]
`}
  caption="Mutations always write locally first. The network step happens afterward, and the result is either an acknowledgement or a stored conflict."
/>
...
- [Plugins](/docs/reference/client-sdk/javascript/plugins) - Auto-versioning plugin
