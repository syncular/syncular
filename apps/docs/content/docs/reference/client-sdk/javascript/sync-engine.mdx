...
The sync engine provides fine-grained control over the sync process. These are low-level building blocks -- most applications should use the [`Client` class](/docs/reference/client-sdk/javascript/overview) instead.
...
stateDiagram-v2
  [*] --> IDLE: start()
  IDLE --> SYNCING: poll / wake-up / sync()
  SYNCING --> IDLE: success
  SYNCING --> RETRYING: error
  RETRYING --> SYNCING: retry timer
  IDLE --> STOPPED: stop() / destroy()
  SYNCING --> STOPPED: stop() / destroy()
  RETRYING --> STOPPED: stop() / destroy()

  state "Realtime transport" as Realtime {
    [*] --> disconnected
...
`}
  caption="The engine cycles between idle, syncing, and retrying. Realtime connection state is tracked separately from the sync loop."
/>
...
- [Mutations](/docs/reference/client-sdk/javascript/mutations) - High-level mutation API
