...
For most non-React apps, start with `createClient()`. Reach for `new Client(...)` when you need explicit control over transport construction, subscriptions, or lifecycle wiring.

### What you'll learn

- How to install and configure the `Client` class
- When to use `createClient()` instead of `new Client(...)`
...
Manual `Client` setup when you want full wiring control:
...
interface ClientOptions<DB> {
  ...
  plugins?: SyncClientPlugin[];     // Optional: additional plugins; incrementing-version is included by default
...
Remote/synced `data:change` emissions are debounced by default (`10ms`). Pass
`0` or `false` to any debounce option to disable debouncing for that mode.
Local mutation invalidations are always emitted immediately.
...
```typescript
import { Client } from '@syncular/client';
import { createBlobPlugin } from '@syncular/client-plugin-blob';
...
const client = new Client({
  ...
  actorId: 'user-456',
  subscriptions: [...],
  plugins: [createBlobPlugin({ storage: blobStorage })],
});
...
- [Mutations](/docs/reference/client-sdk/javascript/mutations) - Making changes
- [Plugins](/docs/reference/client-sdk/javascript/plugins) - Extending functionality
