...
For the standard `server_version` flow, both `useMutations()` and `useMutation()` automatically read the current local row version for updates and deletes when `baseVersion` is omitted. Pass `baseVersion` to override that value, or `null` to disable optimistic concurrency for a specific write.
...
function TaskItem({ task }) {
  ...
  const updateTitle = async (newTitle: string) => {
    await mutate.upsert(task.id, { title: newTitle });
  };
