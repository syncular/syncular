...
Subscribe to a selected slice of engine state with custom equality.

Use this when you want finer control than `useSyncStatus`, `useSyncConnection`, or `useTransportHealth`, or when a component should react to only one small part of the engine snapshot.
...
`useTransportHealth` is a specialized selector hook for realtime fallback state. If you need a different transport-derived slice, use `useSyncSelector`.
