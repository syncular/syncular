...
Syncular uses **pre-defined table handlers** instead. The client requests data by table name and scope values; the server validates access via `resolveScopes`. For details on how scope validation works, see [Scopes](/docs/concepts/scopes).
...
The server intersects the requested scope values with the actor's allowed scopes. See [Scopes: The Intersection Model](/docs/concepts/scopes#effective-scopes-the-intersection-model) for how this works.
...
When a subscription is removed or revoked, the system clears locally cached
data for that subscription. The default client handler uses the handler's
`scopes` mapping to build scoped deletes:
...
With that handler, clearing `{ user_id: '123' }` deletes rows where
`owner_user_id = '123'`. Empty scopes (`{}`) mean a global subscription, so the
default clears the whole table.

If the handler has no local mapping for a non-empty scope, Syncular throws
instead of falling back to a whole-table delete. Add a scope mapping or provide
custom clearing:

```typescript
const tasksClientHandler = create generated client subscriptions({
  table: 'tasks',
  scopes: ['user:{user_id}'],
...
Custom `clearAll` is only needed when your local clearing rule is more complex
than direct scope-column equality, or when you intentionally do not store local
scope columns.
...
- [Scopes](/docs/concepts/scopes) -- How scopes control authorization
- [Commits](/docs/concepts/commits) -- The atomic unit of change
