...
### Server rows may omit local scope metadata

It is often good API hygiene to hide ownership columns from public row payloads.
When you do that, keep the local scope column and opt into materialization:
...
`materializeScopeColumns` fills missing local scope columns from the subscription
scope during snapshots and from each change's stored scopes during incremental
pulls. Snapshot materialization requires a single concrete value for each
missing scope column. If a subscription requests multiple values at once, include
the scope column in the server snapshot rows or split the subscription by value.
