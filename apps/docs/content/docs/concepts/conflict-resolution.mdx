...
High-level stateful clients already include the incrementing version plugin by default:
...
You only need to wire it manually when you are composing low-level mutation/push helpers yourself.
...
await client.sync(db, transport, handlers, {
  ...options,
  plugins: [plugin],
});
```

This plugin does **not** read local rows or turn on conflict detection by itself.

What it actually does:

- leaves inserts and missing `base_version` values alone
- tracks the next expected version after successful pushes
- bumps already-present numeric `base_version` values forward for rapid sequential writes on the same row

That prevents "self-conflicts" when one client enqueues several updates to the same row in quick succession.

For the standard `server_version` flow, `useMutations()` and `useMutation()` already read the current local row version automatically unless you override `baseVersion` yourself.
...
- [Commits](/docs/concepts/commits) -- How the commit log works
- [Glossary](/docs/concepts/glossary) -- Key terms (Conflict, Outbox, Scope)
- [Table Handlers](/docs/reference/server/table-handlers-reference) -- Implement server-side conflict detection
