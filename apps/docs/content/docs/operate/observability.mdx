...
This guide is about making Syncular visible in production: what gets emitted, how to wire a backend, and how to connect sync behavior to the rest of your incident tooling.

## Use this page when

- you are moving from local development to a real deployment
- you need logs, traces, metrics, or exceptions in an existing observability stack
- you want to debug sync incidents without guessing from user reports

## What you'll learn

- what Syncular emits by default
- how the pluggable telemetry contract is shaped
- how to wire Sentry or a custom backend
- how to emit app-specific telemetry alongside Syncular telemetry

## Prerequisites

- a working Syncular server or client setup
- familiarity with your target observability backend
- access to the deployment environment where telemetry configuration lives
...
- [Troubleshooting](/docs/operate/troubleshooting) for incident debugging workflows
- [Performance](/docs/operate/performance) for bootstrap, prune, and compaction tuning
- [Console Stream Investigation](/docs/operate/console/stream-investigation) for operator-side request forensics
- [Testing](/docs/testing) if you want to validate failures before production
...
- Browser init: configure diagnostics through `@syncular/client`
- Browser Sentry options: forward client diagnostic events into your app sink
- Service worker server init: expose the Hono/server routes used by your app
- Cloudflare worker init: wire `@syncular/server-cloudflare` routes in the Worker
