...
- you need a deployment shape for Docker, Workers, fly.io, Railway, or similar targets
- you are building a production checklist around health checks, CORS, and environment configuration
...
- A working Syncular server setup ([Server Setup](/docs/server/setup-with-hono))
...
Syncular works on Cloudflare Workers with some considerations. See the [Cloudflare Adapter](/docs/reference/server/cloudflare-adapter) page for the full adapter setup.
...
For multi-instance deployments, configure a realtime broadcaster so WebSocket delta and pull-required events reach all instances:
...
- [Server Setup](/docs/server/setup-with-hono) - Configure sync routes and handlers
