...
- you are planning pruning, compaction, or large-dataset behavior before launch
- you want realistic defaults for catalogs, long-lived clients, or high write volume
...
- A working Syncular setup ([Server Setup](/docs/server/setup-with-hono), [Client Setup](/docs/clients/rust/quick-start))
...
Syncular uses **snapshot chunks**: the server encodes rows with `binary-table-v1`, compresses the payload with gzip, caches it, and returns chunk references in the pull response. The client then downloads chunks separately via `GET /sync/snapshot-chunks/:chunkId`.

The built-in HTTP transport automatically sends the subscription scope values back on chunk fetches via `X-Syncular-Snapshot-Scopes`. If you fetch chunk URLs manually, include that header with the same scope values from the pull response; chunk IDs are not treated as standalone bearer tokens.
...
const syncular = await createSyncularClient<AppDb>({
  config: {
    ...
    clientId,
    pull: {
      // Rows per snapshot page, capped by maxPullLimitSnapshotRows
      limitSnapshotRows: 1000,
      // Pages per pull cycle, capped by maxPullMaxSnapshotPages
      maxSnapshotPages: 5,
...
The Rust-owned SQLite runtime installs app tables and generated indexes from
your migrations/app contract. Add indexes to your client SQLite migrations so
the generator can carry the same schema contract to JavaScript and Rust
clients:
...
```tsx
// Good: scoped subscriptions in your generated app client
await syncular.client.setSubscriptions([
  {
    id: `notes:user:${userId}`,
    table: 'notes',
...
// Avoid: a broad subscription that pulls far more rows
await syncular.client.setSubscriptions([
  {
    id: 'all-notes',
    table: 'notes',
...
Model default scopes in `syncular.app.ts` so generated helpers create the common
subscriptions:
...
Syncular's `useSyncQuery` hook uses fingerprint-based diffing plus structural sharing to avoid unnecessary React rerenders. Single-table keyed lists use row-level mutation timestamps; multi-table queries automatically fall back to value-based fingerprinting so joined data stays correct. Ensure your queries include an `id` column (or configure `keyField`) for the fastest single-table path:

```typescript
const { data } = useSyncQuery(
  ({ selectFrom }) =>
    selectFrom('notes').selectAll().orderBy('created_at', 'desc'),
...
Realtime is the canonical wake-up path. Interval polling is opt-in for apps
that need an additional background cadence:
...
When running multiple server instances behind a load balancer, WebSocket realtime events only reach the instance that received the push. Use a realtime broadcaster to distribute events:
...
1. **Enable realtime WebSocket** for verified deltas and pull-required wake-ups
...
await syncular.client.setSubscriptions([
  ...
  ...workspaceIds.map((workspaceId) => ({
    id: `notes-workspace-${workspaceId}`,
    table: 'notes',
...
for (let i = 0; i < allItems.length; i += BATCH_SIZE) {
  ...
  await syncular.mutations.$commit(async (tx) => {
    await tx.notes.insertMany(
      batch.map((item) => ({
        id: item.id,
        user_id: item.userId,
        title: item.title,
...
Use `useSyncConnection` and `useOutboxStats` to surface client metrics:

```typescript
const connection = useSyncConnection();
// connection.state.realtime -- websocket state
// connection.state.pendingRequests -- in-flight client requests

const outbox = useOutboxStats();
// outbox?.pending -- pending commits
// outbox?.failed -- failed commits
// outbox?.total -- total outbox size
```

## Next Steps

- [Deployment](/docs/operate/deployment) - Deploy to production
- [Schema Migrations](/docs/features/migrations) - Evolve your schema
- [Realtime](/docs/server/realtime-websocket) - Configure WebSocket realtime
