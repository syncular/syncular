...
description: Mount operations routes, serve Console, and configure maintenance behavior.
---
...
For UI/operator workflows, see the dedicated [Console docs](/operate/console). This page focuses on route setup and endpoint behavior.
For integration options (CLI serve, mount API, components), see [Console Integration](/operate/console-integration).

## Setup

### Simple path: `createSyncServer`

If you build your server with `createSyncServer`, console routes are one
option away — the event emitter, schema setup, and WebSocket wiring are
handled for you:
...
const { syncRoutes, consoleRoutes } = createSyncServer({
  ...
  upgradeWebSocket,
  console: {
    token: process.env.SYNC_CONSOLE_TOKEN, // or rely on the env var directly
...
### Full control: `createConsoleRoutes`

For custom console auth or separate deployment of the console API, create the
routes yourself. Pass the same event emitter to your sync routes
(`consoleLiveEmitter`) — that is what records request events and feeds the
live feed:
...
const syncRoutes = createSyncRoutes({
  ...
  authenticate,
  consoleLiveEmitter: eventEmitter,
});
...
All paths below are **relative to where you mount** the console routes
(example above mounts at `/api/console`).
...
const consoleGatewayRoutes = createConsoleGatewayRoutes({
  ...
  instances: [
    ...
    },
    {
      instanceId: 'archive',
      label: 'Archive',
      baseUrl: 'https://api.acme.dev/internal/archive-sync',
      token: process.env.ARCHIVE_CONSOLE_TOKEN, // optional override
    },
  ],
  websocket: {
    enabled: true,
...
interface ConsoleCommitDetail {
  ...
  changes: Array<{
    ...
    table: string;
    rowId: string;
    // Console exposes the stored protocol operation marker.
    // App/client APIs should still use insert/update/delete.
    op: 'upsert' | 'delete';
    rowJson: unknown;
...
Clients opt into publishing runtime diagnostics
(`consoleDiagnostics` in the client config — see
[Observability](/operate/observability)); the Console's
[Fleet view](/operate/console/fleet) renders them per client.

- **POST /client-diagnostics** — ingestion endpoint the client publisher posts to
- **GET /client-diagnostics** — latest snapshot per client
- **GET /client-diagnostics/:id** — one client's latest snapshot
- **GET /client-diagnostics/:id/history** — recent snapshot history
...
**POST /events/prune** - Apply request + operation event retention and prune unreferenced payload snapshots

Automatic pruning cadence is controlled with `maintenance.autoPruneIntervalMs` on `createConsoleRoutes(...)`. Set it to `0` to disable background pruning and rely only on the endpoint.
...
```typescript
const ws = new WebSocket('wss://api.example.com/api/console/events/live');
...
interface CreateConsoleRoutesOptions {
  ...
  maintenance?: {
    autoPruneIntervalMs?: number;    // Default: 5 minutes
    requestEventsMaxAgeMs?: number;  // Default: 7 days
    requestEventsMaxRows?: number;   // Default: 10000
    operationEventsMaxAgeMs?: number; // Default: 30 days
    operationEventsMaxRows?: number; // Default: 5000
  };
...
```bash
AUTH="Authorization: Bearer $SYNC_CONSOLE_TOKEN"

# Prune old console events immediately (manual trigger)
curl -X POST -H "$AUTH" https://api.example.com/api/console/events/prune

# Compact changes
curl -X POST -H "$AUTH" https://api.example.com/api/console/compact

# Preview, then prune old commits
curl -X POST -H "$AUTH" https://api.example.com/api/console/prune/preview
curl -X POST -H "$AUTH" https://api.example.com/api/console/prune
```

If you enabled `routes.prune` / `routes.compact` on the sync routes
([Deployment](/operate/deployment#pruning-and-compaction)), these run
automatically — the endpoints remain useful for previews and one-off runs.
...
const stats = await fetch('/api/console/stats', { headers }).then((r) => r.json());
const timeseries = await fetch(
  '/api/console/stats/timeseries?interval=minute&range=1h',
  { headers }
).then((r) => r.json());
...
- [Testing Guide](/testing) - Test your sync setup
- [Architecture](/learn/architecture) - System design overview
