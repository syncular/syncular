...
**Typed SQL**: TypeScript apps read through Kysely, Rust apps read through
Diesel, and server handlers use Kysely.
...
- In **Syncular**, you define server-side scope resolution, table handlers, and
  generated subscriptions. Your UI reads via typed SQL (`useSyncQuery` in
  TypeScript, Diesel in Rust) against Rust-owned local SQLite.
...
<Mermaid
  chart={`
flowchart LR
  Client["Client (SQLite)"] <--> Zero["Zero sync layer"]
  Zero <--> Postgres["Postgres"]
`}
/>
...
- You want typed local SQL and explicit generated mutations
