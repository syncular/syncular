...
| **Client** | Rust-owned SQLite with TypeScript, Rust, and native host APIs | TanStack DB, PGlite, any HTTP client |
| **Self-hosted** | Yes | Yes |
...
**Typed SQL**: Kysely reads for TypeScript apps, Diesel reads for Rust apps,
and Kysely-backed server handlers.
...
<Mermaid
  chart={`
flowchart LR
  Client["Client (any)"] <--> Electric["Electric shapes"]
  Electric <--> Postgres["Postgres logical replication"]
  Client -->|writes via API| Api["Your API"]
`}
/>
...
<Mermaid
  chart={`
flowchart LR
  Client["Client (Rust-owned SQLite)"] <--> Server["Your server (Hono)"]
...
- You value typed local SQL plus explicit server handlers
...
*Sources: [Electric SQL docs](https://electric-sql.com/docs), [Writing your own client (proxy/auth guidance)](https://electric-sql.com/docs/reference/client-development/writing-your-own-client), [Electric Shape API](https://electric-sql.com/api/openapi), [Electric GitHub](https://github.com/electric-sql/electric)*
