...
1. Pick the host surface: JavaScript/TypeScript app APIs over the Rust runtime,
   or direct Rust app code.
2. Create your local SQLite schema and generated app contract.
3. Query locally with the host query builder: Kysely for JavaScript or Diesel
   for Rust.
4. Add Syncular server + table handlers.
5. Add scopes + subscriptions for partial sync and auth.
6. Add realtime wakeups when the core push/pull path is proven.
...
- [Admin Database Proxy](/docs/server/admin-proxy)
...
1. Fast first sync for large datasets:
   - [Architecture (bootstrap + snapshot chunks)](/docs/learn/architecture)
...
4. Operations + observability:
   - [Operations](/docs/operate/operations-setup)
   - [Observability](/docs/operate/observability)
5. Relay (intermittent networks / edge):
   - [Relay](/docs/server/edge-relay)
6. Migrations + type generation:
   - [Schema Migrations](/docs/features/migrations)
