...
```bash
syncular migrate up --dry-run --config syncular.config.json
```
...
- `--dry-run`: compute pending migrations without applying
- `--yes`: required when using `--on-checksum-mismatch reset`
