...
| `dataChangeDebounceMs` | `number` | No | Debounce window for coalescing `data:change` events |
| `dataChangeDebounceMsWhenSyncing` | `number` | No | Debounce override while sync is running |
| `dataChangeDebounceMsWhenReconnecting` | `number` | No | Debounce override while reconnecting |
