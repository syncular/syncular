...
// 2. The Rust runtime writes to local SQLite immediately (optimistic)
// 3. An outbox commit is created with one row-write operation
...
App code should not hand-author these operations. Use generated
`insert`/`update`/`delete` mutations so Syncular can record local row state,
outbox metadata, conflict bases, changed fields, blobs, CRDT fields, and
encryption metadata consistently.

### Insert or update

Generated `insert` and `update` mutations become a row-write operation in the
outbox:
...
<Mermaid
  chart={`
flowchart TB
  Edit["Generated mutation"] --> Local["Write through Rust runtime to SQLite"]
...
flowchart TB
  ...
  Decision -->|Yes| Bootstrap["Bootstrap with binary chunks or artifacts"]
  Decision -->|No| Incremental["Incremental commits since cursor"]
  Bootstrap --> Response["Return subscriptions with snapshot descriptors and nextCursor"]
  Incremental --> Response
  Response --> Apply["Runtime verifies and applies to local SQLite"]
...
4. **Return** -- Server returns an array of subscription responses, each with `nextCursor`, `commits`, and optionally snapshot chunk or artifact descriptors.
5. **Apply** -- The Rust client runtime verifies and applies changes to local SQLite inside a transaction.
...
For a deeper dive, see [Scopes](/docs/learn/scopes).
...
- [Architecture](/docs/learn/architecture) -- How commits fit into the sync system
- [Scopes](/docs/learn/scopes) -- Authorization and data partitioning
- [Conflict Resolution](/docs/learn/conflict-resolution) -- What happens when two clients edit the same row
- [Glossary](/docs/learn/glossary) -- Definitions of key terms
