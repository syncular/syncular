...
Whatever you return from `authenticate()` becomes `ctx.auth`. Syncular also
exposes `ctx.actorId` as a convenience alias for `ctx.auth.actorId`. The full
`authenticate()` wiring — custom auth payloads, token verification, rejecting
requests — lives in [Server: Scopes And Auth](/server/scopes-and-auth).
...
## The Client Side: Subscriptions Request Scopes

Scopes have two halves. The server side tags and filters changes; the client
side *requests* scope values through its [subscriptions](/learn/subscriptions):

```typescript
// Client subscription: "give me tasks for project A"
{ id: 'project-a-tasks', table: 'tasks', scopes: { project_id: 'A' } }
```

Generated apps express the same request through subscription helpers, for
example `taskSubscription({ actorId })`. The client never decides what it is
allowed to see — it only asks. The server intersects the request with what
`resolveScopes` allows (see below) before returning anything.
...
For scoped tables, define the same scope shape on the server handler and in the
generated app contract. The server mapping says where scopes come from when
writing the commit log. The generated client mapping says which local column
the Rust runtime should use when a subscription is removed, revoked, or
bootstrapped again.
...
syncedTable({
  table: 'notes',
  scopes: [scope('user_id', { column: 'owner_user_id', source: 'actorId' })],
});
```

If the client subscribes with `{ user_id: 'alice' }`, the local `notes` table
must have the generated `owner_user_id` scope column. Without a local scope
column, Syncular cannot safely clear only Alice's rows during bootstrap or
revocation.

### Server rows should include local scope metadata

For the default generated apply path, synced rows should include the local
columns needed by your client schema. If your local table has `owner_user_id`,
the server snapshot/change payload should normally include `owner_user_id` too,
even if your app UI never displays it.
...
Do not hide required ownership metadata from row payloads unless you also
change the generated app contract and server handler design so the runtime can
still clear revoked scopes precisely. A table with scoped sync but no local
scope-column mapping should fail closed instead of clearing the whole table.

---

## Effective Scopes: The Intersection Model

When a client pulls data, the server computes **effective scopes** as the intersection of what the client requested (via its [subscription](/learn/subscriptions)) and what `resolveScopes` allows:
...
2. A scoped row write for the new scope
...
- [Subscriptions](/learn/subscriptions) -- How clients request data using scopes
- [Server: Scopes And Auth](/server/scopes-and-auth) -- `authenticate()` wiring and server-side authorization checks
- [Data Modeling with Scopes](/features/data-modeling) -- Practical schema + scope design
