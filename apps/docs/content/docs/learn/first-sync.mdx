...
> **Deep dive.** For the canonical “How sync works” overview, start with [Architecture](/learn/architecture).
...
- [Architecture](/learn/architecture) — canonical deep dive into the sync lifecycle
- [Blob Storage](/features/recipes/blobs-and-media) — snapshot chunks use the same storage adapter system
- [Performance](/operate/performance) — bootstrap tuning and large dataset strategy
- [Table Handlers](/server/table-handlers) — controlling snapshot/apply and auth boundaries
