...
> **Deep dive.** For the canonical “How sync works” overview, start with [Architecture](/docs/learn/architecture).
...
For large datasets, Syncular uses **binary snapshot chunks** and scoped
snapshot artifacts instead of embedding large row arrays in the sync response:
- The server frames rows into a compact binary format, compresses (e.g. gzip),
  and stores chunks using the same **blob storage adapter** system.
- The sync response returns chunk descriptors (id/hash/encoding/compression) instead of embedding all rows inline.
- The Rust client runtime downloads chunks, verifies hashes, decodes rows, and
  applies them to SQLite.

This enables very fast first syncs:
- Less JSON overhead in the bootstrap response
...
On the client, the generated app contract tells the Rust runtime how to apply
bootstrap snapshots and incremental changes.

Defaults are designed to work well out of the box:
- Snapshot apply: write the received rows by primary key inside a transaction.
- Change apply: insert, update, or delete by primary key after decoding server
  changes.
- Scope revocation: clear data for scopes that are no longer subscribed or
  allowed.

Most apps should not customize runtime apply behavior. Model soft delete, stale
marking, and derived projections in generated table metadata or local read
models so the runtime can keep outbox, conflicts, CRDT fields, and live events
coherent.
...
- [Architecture](/docs/learn/architecture) — canonical deep dive into the sync lifecycle
- [Blob Storage](/docs/features/recipes/blobs-and-media) — snapshot chunks use the same storage adapter system
- [Performance](/docs/operate/performance) — bootstrap tuning and large dataset strategy
- [Table Handlers](/docs/server/table-handlers) — controlling snapshot/apply and auth boundaries
