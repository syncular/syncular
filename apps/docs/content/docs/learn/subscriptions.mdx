...
Syncular uses **pre-defined table handlers** instead. The client requests data by table name and scope values; the server validates access via `resolveScopes`. For details on how scope validation works, see [Scopes](/docs/learn/scopes).
...
The server intersects the requested scope values with the actor's allowed scopes. See [Scopes: The Intersection Model](/docs/learn/scopes#effective-scopes-the-intersection-model) for how this works.
...
- [Scopes](/docs/learn/scopes) -- How scopes control authorization
- [Commits](/docs/learn/commits) -- The atomic unit of change
