...
export function toSvg(
  ...
  const spans = cells
    .map(
      (c) =>
        `<tspan x="${c.cx.toFixed(1)}" y="${c.cy.toFixed(1)}" fill="${t[c.cls]}">${esc(c.ch)}</tspan>`,
    )
    .join('');
...
export function singularity({
  ...
  const cells = [];
  const rDisk = CY - 0.5;
  const rHorizon = rDisk * horizonFrac,
    rRing = rDisk * ringFrac;
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      if (rr < rHorizon) {
        ...
        let b =
          (0.5 + 0.5 * Math.sin(2 * a - rr * radialFrequency + diskPhase)) *
          (1 - band * 0.8);
  ...
  for (const { col, row, text } of labels) {
    ...
    for (let i = 0; i < text.length + 2; i++) {
      ...
      const idx = cells.findIndex(
        (c) =>
          Math.abs(c.cx - cxp) < cw * 0.4 && Math.abs(c.cy - py) < ch * 0.4,
      );
...
export function wordmark(spec, t, fontCss = '') {
  ...
  const cw = spec.fs * 0.6,
    ch = spec.fs * 1.05;
  const markW = spec.cols * cw,
    markH = spec.rows * ch;
  ...
  const spans = s.cells
    .map(
      (c) =>
        `<tspan x="${(pad + c.cx).toFixed(1)}" y="${(pad + c.cy).toFixed(1)}" fill="${t[c.cls]}">${esc(c.ch)}</tspan>`,
    )
    .join('');
...
if (import.meta.main) {
  ...
  for (const theme of ['dark', 'light']) {
    writeFileSync(
      `${OUT}/4-wordmark-${theme}.svg`,
      wordmark(COARSE.wordmark, P[theme]),
    );
  }
