...
[`../migration record`](../migration record) — read that first. This file is the rules of
the tree.
...
- Its **own workspace root**: own lockfile, **latest Bun** (no version pin —
  ...
  `.github/workflows/v2.yml` on `v2/**` paths.
...
## Rules (from migration record, enforced in review)
...
4. **Tripwire**: two clients must be syncing through the TS server within
   ~2 weeks of agent-time, or work stops and migration record's gate is evaluated
   early.
