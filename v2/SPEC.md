...
*New in v2 (work item 2.1, the first parity-ladder rung).* Blobs are opaque
...
*New in v2 (work item 2.2, the second parity-ladder rung).* A `crdt` column
...
or `baseVersion`-conflicting. This is the hybrid-consistency pillar (migration gate):
...
(migration gate rule 3) and not part of core conformance. Reserved for it: the
