...
describe('DbMetadataSnapshotChunkStorage', () => {
  ...
  describe('storeChunk', () => {
    it('stores chunk metadata in DB and body in blob adapter', async () => {
      ...
      expect(ref.encoding).toBe('ndjson');
      expect(ref.compression).toBe('gzip');

      // Verify body stored in blob adapter (hash computed from body content)
      const { createHash } = await import('node:crypto');
      const computedHash = `sha256:${createHash('sha256').update(body).digest('hex')}`;
    ...
    it('deduplicates blobs with same content', async () => {
      ...
      // But blob should only be stored once (computed hash from body)
      const { createHash } = await import('node:crypto');
      const computedHash = `sha256:${createHash('sha256').update(body).digest('hex')}`;
    ...
    it('updates existing chunk on conflict (same page key)', async () => {
      ...
      const { createHash } = await import('node:crypto');
      const oldBlobHash = `sha256:${createHash('sha256').update(body1).digest('hex')}`;
      const oldBlob = await mockBlobAdapter.get(oldBlobHash);
      expect(oldBlob).not.toBeNull();

      // New blob should exist (computed hash from body2)
      const newBlobHash = `sha256:${createHash('sha256').update(body2).digest('hex')}`;
  ...
  describe('cleanupExpired', () => {
    it('removes expired chunks and their blobs', async () => {
      ...
      const expiredBlob = await mockBlobAdapter.get('sha256:expired123');
