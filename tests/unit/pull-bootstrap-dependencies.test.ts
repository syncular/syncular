...
describe('pull bootstrap behavior', () => {
  it('bootstraps dependency tables before the requested shape', async () => {
    const db = createBunSqliteDb<ServerDb>({ path: ':memory:' });
    ...
    try {
      ...
      shapes.register(projectsHandler);
      ...
      expect(decodeNdjsonGzip(projectChunk.body)).toEqual([
        { id: 'p1', user_id: 'u1', name: 'Project 1', server_version: 1 },
      ]);
      expect(decodeNdjsonGzip(taskChunk.body)).toEqual([
        {
          id: 't1',
          user_id: 'u1',
          project_id: 'p1',
