...
export async function runSubscriptionScenario(
  ...
  // Pull incremental
  const inc = await syncPullOnce(client.db, client.transport, client.shapes, {
    clientId: client.clientId,
    subscriptions: [subP1],
  });
...
export async function runAddSubscriptionScenario(
  ...
  // First: sync only p1
  const first = await syncPullOnce(client.db, client.transport, client.shapes, {
    clientId: client.clientId,
    subscriptions: [subP1],
  });
...
export async function runDedupeScenario(ctx: ScenarioContext): Promise<void> {
  ...
  const res = await syncPullOnce(client.db, client.transport, client.shapes, {
    clientId: client.clientId,
    subscriptions: [subP1],
    dedupeRows: true,
    limitCommits: 50,
...
export async function runForcedBootstrapAfterPruneScenario(
  ...
  // Create 10 commits
  for (let i = 1; i <= 10; i++) {
    await fastClient.transport.push({
