...
import type { SyncPushRequest, SyncTransport } from '@syncular/core';
    ...
    switch (action.type) {
      ...
      case 'push': {
        ...
        for (const commit of commits) {
          ...
          const response = await client.transport.push(request);

          // Update outbox status based on response
          const newStatus =
      ...
      case 'pull': {
        ...
        const response = await client.transport.pull({
          clientId: client.id,
          limitCommits: 100,
