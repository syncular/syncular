...
describe('user resolver overrides', () => {
  it('accepts string override', () => {
    const resolver = () => 'Date';
    const result = resolveColumnType(col({ sqlType: 'TEXT' }), resolver);
  ...
  it('accepts object override without import', () => {
    const resolver = () => ({ type: 'Record<string, number>' });
    const result = resolveColumnType(col({ sqlType: 'TEXT' }), resolver);
    expect(result.tsType).toBe('Record<string, number>');
    expect(result.imports).toEqual([]);
  });

  it('falls through to default when resolver returns undefined', () => {
  ...
  it('resolver receives correct ColumnInfo', () => {
    let received: ColumnInfo | undefined;
    const resolver = (c: ColumnInfo) => {
      received = c;
      return undefined;
    };
    resolveColumnType(
  ...
  it('resolver for specific table.column combo', () => {
    const resolver = (c: ColumnInfo) => {
      if (c.table === 'tasks' && c.column === 'metadata') {
        return {
          type: 'TaskMeta',
          import: { name: 'TaskMeta', from: './types' },
        };
      }
      return undefined;
    };
    ...
    // Matching column
    const result1 = resolveColumnType(
      col({
        table: 'tasks',
        column: 'metadata',
        sqlType: 'jsonb',
    ...
    expect(result1.tsType).toBe('TaskMeta');
    expect(result1.imports).toHaveLength(1);

    // Non-matching column falls through
    const result2 = resolveColumnType(
      col({
        table: 'users',
        column: 'metadata',
        sqlType: 'jsonb',
  ...
  it('resolver wraps nullable override', () => {
    const resolver = () => 'Date';
    const result = resolveColumnType(
      col({ sqlType: 'TEXT', nullable: true }),
      resolver
    );
