 ...
 *   - SEED_ROWS: Total rows to seed across all users (default: 10000)
...
async function seedData(
  ...
  const rowsPerUser = Math.ceil(totalRows / userCount);
  const batchSize = 1000;

  console.log(
    `Seeding ${totalRows} rows across ${userCount} users (${rowsPerUser} per user)...`
  );

  const startTime = Date.now();

  for (let userIndex = 0; userIndex < userCount; userIndex++) {
    // Use different prefixes for different test scenarios
    const userPrefixes = [
      'user',
      'reader',
      'writer',
    ...
    // Progress indicator
    if ((userIndex + 1) % 10 === 0 || userIndex === userCount - 1) {
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      console.log(`  Seeded ${userIndex + 1}/${userCount} users (${elapsed}s)`);
    }
  }

  const totalElapsed = ((Date.now() - startTime) / 1000).toFixed(1);
  console.log(`Seeding complete in ${totalElapsed}s`);
}
...
async function main() {
  ...
  // Seed data
  if (seedRows > 0) {
    await seedData(db, seedRows, seedUsers);
  }
