...
// ---------------------------------------------------------------------------
// node:sqlite → Kysely adapter
//
// Kysely's SqliteDialect expects better-sqlite3 API: stmt.all(params[])
// node:sqlite uses: stmt.all(...params). This adapter bridges the difference.
// ---------------------------------------------------------------------------

function wrapStatement(stmt: ReturnType<sqlite.DatabaseSync['prepare']>) {
  // Detect if the statement returns data (SELECT, PRAGMA, etc.)
  // node:sqlite's columns() throws for non-reader statements (DDL, INSERT, etc.)
  let isReader = false;
  try {
