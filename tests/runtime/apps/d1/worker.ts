...
async function runConformance(env: Env): Promise<void> {
  const db = createD1Db<ConformanceDb>(env.DB);
  try {
    // Create schema
    await db.schema
    ...
    assert(
      row.bytes instanceof Uint8Array || row.bytes instanceof ArrayBuffer,
      'unexpected bytes type'
    );
    assert(
...
async function createSyncClient(env: Env, serverUrl: string, actorId: string) {
  const db = createD1Db<RuntimeClientDb>(env.DB);
  ...
  return { db: db as Kysely<RuntimeClientDb>, shapes, transport };
}
